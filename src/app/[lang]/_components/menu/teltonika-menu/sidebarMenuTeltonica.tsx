...
function MenuBloc({ Icon, title, items }: BlocType) {
  const [sidebarMenuState, setSidebarMenuState] = useRecoilState(sidebarMenuStateAtom);
    ...
    <div className=" w-full flex flex-row">
      ...
      >
        <div className="w-full ">
          <Icon className=" w-full h-12 py-2 text-white hover:bg-teltonika-900" />
        </div>
      </div>
      <div
        className={` bg-white ${
      ...
      >
        ...
        <div className="mt-2 flex flex-col">
          {items.map((item) => (
            <MenuBlocItem name={item.name} link={item.link} />
          ))}
        </div>
      </div>
    </div>
...
function MenuBlocSmScreen({ Icon, title, items }: BlocType) {
  const [sidebarMenuState, setSidebarMenuState] = useRecoilState(sidebarMenuStateAtom);
    ...
    <>
      ...
      >
        ...
        <div className="w-full flex flex-col h-full ">
          <div className=" w-full flex flex-row ">
            <div
              className={`bg-teltonika-800 w-12 h-12  flex items-start justify-center cursor-pointer`}
            ></div>
            <div
              className={`bg-white  ${
                sidebarMenuState ? " w-52 px-5 py-3" : "w-0 h-0 overflow-hidden"
              }`}
            ></div>
          </div>
          ...
          <div className="flex flex-row flex-grow ">
            ...
            >
              <div className="w-full ">
                {sidebarMenuState ? <TbChevronsLeft
                  onClick={toggleMenu}
...
{/* sm screen */}
      <div
        className={`absolute bg-gray-100 bottom-0 z-30 w-screen  lg:hidden transition-all duration-300 ease-out  ${
          sidebarMenuState ? " h-[94%]" : "h-0 w-0 overflow-hidden "
        }`}
      >
         <div className="w-full flex flex-col h-full ">
          {/* <div className=" w-full flex flex-row ">
            <div
              className={` w-12 h-2  flex items-start justify-center cursor-pointer`}
            ></div>
            <div
              className={`bg-gray-100  ${
                sidebarMenuState ? " w-52 px-5 py-3" : "w-0 h-0 overflow-hidden"
        ...
        {/*  {sidebarMenuState ? (
          <div className="w-full h-full flex items-center justify-start pl-3">
            <VscChromeClose
              ...
              className="w-8 hover:scale-110  h-8 transition-all duration-300 ease-out cursor-pointer  text-teltonika-800"
            />
          </div>
        ) : (
          <div className="w-full h-full flex items-center justify-start pl-3">
